export const mainCode = `import os
import sys
...
def read_typescript_files(directory: str) -> List[Tuple[str, str]]:
    ...
    log.info(f"[green]Read {len(typescript_files)} relevant files[/green]")
...
def create_code_screenshot(file_name: str, code_content: str, caption: str, output_dir: str) -> str:
    try:
        ...
        new_image = Image.new('RGB', (code_image.width, code_image.height + 0), color='#272822')
        
        # Paste the code image onto the new image
        new_image.paste(code_image, (0, 0))
...
def resize_image(image_path, max_width_inches=6.5, max_height_inches=9):
    with Image.open(image_path) as img:
        ...
        max_width_pixels = int(max_width_inches * 96)  # 96 DPI
        max_height_pixels = int(max_height_inches * 96)  # 96 DPI
        
        new_width = min(width, max_width_pixels)
        new_height = int(new_width / aspect_ratio)
        
        if new_height > max_height_pixels:
...
def create_word_document(explanations: List[Tuple[str, str, str, str]], output_dir: str):
    try:
        ...
        if isinstance(figure_style, _ParagraphStyle):
        ...
        for file_name, caption, explanation, screenshot_path in explanations:
            ...
            if isinstance(heading_style, _ParagraphStyle):
            ...
            if os.path.exists(full_screenshot_path):
                ...
                log.warning(f"Screenshot not found for {file_name}. Skipping image.")
            ...
            if explanations.index((file_name, caption, explanation, screenshot_path)) < len(explanations) - 1:
                doc.add_paragraph()  # Add a single line space
        
        # Save the document
        output_path = os.path.join(output_dir, "code_explanations.docx")
        doc.save(output_path)
        log.info(f"Word document created with auto-numbered figures: {output_path}")
        return output_path
    except Exception as e:
        log.error(f"Error creating Word document: {str(e)}")
        return None
