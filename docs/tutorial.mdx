...
1. Create a new directory for the project and navigate into it:<br /><br /><CodeHighlighter language="bash" code={`mkdir code-explanation-generator
cd code-explanation-generator`} />

2. Set up a virtual environment:<br /><br />
For Windows:<br /><CodeHighlighter language="bash" code={`python -m venv venv
.\\venv\\Scripts\\activate`} /><br />For macOS and Linux:<br /><CodeHighlighter language="bash" code={`python3 -m venv venv
source venv/bin/activate`} /><br />You should now see `(venv)` at the beginning of your command prompt, indicating that the virtual environment is active.

3. Create a file named `main.py` and copy the source code into it <br /><br /><CodeHighlighter language="python" code={mainCode} />

4. Create a file named `requirements.txt` and copy the following dependencies into it:<br /><br /><CodeHighlighter language="text" code={requirementTxt} />

5. Install the required dependencies:<br /><br /><CodeHighlighter language="bash" code="pip install -r requirements.txt" />

6. Set up your environment variables:
   Create a `.env` file in the project root and add the following:<br /><br /><CodeHighlighter language="bash" code={env} />

7. When you're done working on the project, you can deactivate the virtual environment by running:

<CodeHighlighter language="bash" code="deactivate" />
...
<CodeHighlighter language="text" code={structure} />
